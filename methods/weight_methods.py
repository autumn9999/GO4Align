        ...
        **kwargs,
        ...
        if (self.step % self.update_weights_every) == 0:
            ...
            for i, loss in enumerate(losses):
                ...
                grads[i] = grad  # 44117184

            G = torch.stack(tuple(v for v in grads.values()))  # 3 * 44117184
            GTG = torch.mm(G, G.t())  # 3 * 3
        ...
        # -----------------
        # updated_gradient = alpha.unsqueeze(1).float().cuda() * G
        # updated_overall = updated_gradient.sum(0)
        # # print(updated_overall.norm() - np.sqrt(3))
        # projection = updated_overall @ torch.transpose(updated_gradient, 0, 1)
        # # print(projection)
        # # cosine = projection / (updated_overall.norm() * updated_gradient.norm(dim=1))
        # norm_gradient = updated_gradient / updated_gradient.norm(dim=1).unsqueeze(1)
        #
        # norm_gradient @ torch.transpose(norm_gradient, 0, 1)
        # norm_G = G / G.norm(dim=1).unsqueeze(1)
        # norm_G @ torch.transpose(norm_G, 0, 1)
        # -----------------
...
class FAMO(WeightMethod):
    ...
    def get_weighted_loss(self, losses, **kwargs):
        ...
        return loss, dict(weights=torch.cat([z]))
...
METHODS = dict(
    ...
    group_sdp_clustering=GROUP_sdp_clustering,
