...
common_parser.add_argument("--method", type=str, choices=list(METHODS.keys()), help="MTL weight method")
...
def extract_weight_method_parameters_from_args(args):
    ...
    weight_methods_parameters.update(
        dict(
            nashmtl=dict( update_weights_every=args.update_weights_every, optim_niter=args.nashmtl_optim_niter),
            ...
            group=dict(task_weights=args.task_weights, robust_step_size=args.robust_step_size),
            group_random=dict(task_weights=args.task_weights, robust_step_size=args.robust_step_size),
            #-------------rebuttal
            group_sklearn_spectral_clustering_cluster_qr=dict(num_groups=args.num_groups, robust_step_size=args.robust_step_size),
            group_sklearn_spectral_clustering_discretize=dict(num_groups=args.num_groups, robust_step_size=args.robust_step_size),
            group_sklearn_spectral_clustering_kmeans=dict(num_groups=args.num_groups, robust_step_size=args.robust_step_size),
            group_sdp_clustering=dict(num_groups=args.num_groups, robust_step_size=args.robust_step_size),
        )
    )
